...
function! omni#cpp#items#Get(tokens, ...)
    ...
    call omni#common#debug#Trace("Dot", tokens)
...
function! omni#cpp#items#ResolveItemsTypeInfo(contextStack, items)
    ...
    let bSearchDecl = 1
    call omni#common#debug#Trace("ResolveItemsTypeInfo", a:contextStack)
    for item in a:items
        call omni#common#debug#Trace("ResolveItemsTypeInfo", item)
        let curItem = item
